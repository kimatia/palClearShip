...
if($_SESSION['file']==$BOL){
  ...
  {
  $validextensions = array("jpg", "png", "gif", "bmp", "jpeg", "GIF", "JPG", "PNG", "doc", "txt", "docx", "pdf", "xls", "xlsx");
    ...
    if(!$SQL)
    {
     ...
     if(!$SQL)
     {
      ...
      $userId = $_POST['userId'];
            ...
            $action1="posted BOL document No.";
            $action2="of file No.";
            $action3="owned by";
...
// select bol information to prove if file was instantiated with bol file
    ...
    if($SQL){
      $SQL->bind_param('sssiss',$stackNumber,$ecertNumber, $ecert_file, $userId,$postTime,$postDate);
      $SQL->execute();
        $finalDateFormat=date("y:m:d",time()); 
        $mergeDate="20";
        $finalTime=date("h:i A.",time());
...
// select bol information to prove if file was instantiated with bol file
    ...
    if($bolSelect['bol']=="No"){
      echo "<div class='alert alert-danger'>
              <button class='close' data-dismiss='alert'>&times;</button>
              <strong>Error!</strong>.Please insert Bill of Lading data first then proceed to Invoice File.</div>";
    }elseif($bolSelect['bol']=="Yes"){
    if($SQL){
      ...
      $SQL = $con->prepare("INSERT INTO tbl_logs(userId,stackNumber,file, postTime,postDate) VALUES(?,?,?,?,?)");
          ...
          if(!$SQL){
            ...
            $respFileMessage = $con->query("SELECT * FROM tbl_users WHERE userID='$userId'");
             ...
             $postDate=$mergeDate."".$postDateFormat;
            $userMessage=$rowFileMessage['userName'];
            $action1="posted KRA Receipt document No.";
            $action2="of file No.";
            $action3="owned by";
            // select bol information to prove if file was instantiated with bol file
    $stmt_select_owner = $DB_con->prepare('SELECT fileOwner FROM tbl_stacks WHERE stackNumber=:stack');
